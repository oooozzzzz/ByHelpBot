...
const DB_PATH =
  process.env.DB_PATH ||
  path.resolve(__dirname, "../../checkpointer/checkpoints.db");
...
function runAsync(sql, params = [], dbRef = dbInstance) {
  if (!dbRef) {
    return Promise.reject(
      new Error(
        "База данных не инициализирована. Вызовите initializeDb() перед использованием."
      )
    );
  }
...
CREATE TABLE IF NOT EXISTS writes (
  ...
  PRIMARY KEY (thread_id, checkpoint_ns, checkpoint_id, task_id, idx)
          ...
          .catch((setupErr) => {
            console.error(
              "Ошибка во время инициализации или настройки БД:",
              setupErr.message
            );
            if (dbInstance) {
              dbInstance.close((closeErr) => {
                if (closeErr)
                  console.error(
                    "Ошибка при закрытии БД после ошибки настройки:",
                    closeErr.message
                  );
                else
                  console.log(
                    "Соединение с БД закрыто из-за ошибки настройки."
                  );
              });
            }
            dbReadyPromise = null;
...
function getDbInstance() {
  if (!dbReadyPromise) {
    console.warn(
      "getDbInstance() вызвана до явного initializeDb(). Инициирую."
    );
    return initializeDb();
  }
  return dbReadyPromise;
